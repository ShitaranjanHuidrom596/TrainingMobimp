    ...
    public partial class MachineMaster : BasePage
    {
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Messagealert_.ShowMessage(lblmessage, "Please enter machine name.", 0);
                    divmsg1.Visible = true;
                    divmsg1.Attributes["class"] = "FailAlert";
        ...
        private void bindgrid(int page)
        {
            try
            {
                ...
                List<MachineMasterData> lstemp = GetLabMachineType(page);

                if (lstemp.Count > 0)
                {
                    GvLabMachineType.VirtualItemCount = lstemp[0].MaximumRows;//total item is required for custom paging
                    GvLabMachineType.PageIndex = page - 1;
        ...
        private void ExportToPdf()
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    GvLabMachineType.AllowPaging = false;
                    GvLabMachineType.DataSource = GetLabMachineDetails(0);
                    GvLabMachineType.DataBind();
        ...
        {
            List<MachineMasterData> DesignationTypeDetails = GetLabMachineDetails(0);
        ...
        private List<MachineMasterData> GetLabMachineDetails(int p)
        {
            MachineMasterData objData = new MachineMasterData();
            MachineMasterBO objBO = new MachineMasterBO();
            objData.Code = txtcode.Text == "" ? "" : txtcode.Text;
            objData.MachineName = txtmachinename.Text == "" ? "" : txtmachinename.Text;
            objData.IsActive = ddlstatus.SelectedValue == "0" ? true : false;
            return objBO.SearchLabMachineTypeDetails(objData);
        }
