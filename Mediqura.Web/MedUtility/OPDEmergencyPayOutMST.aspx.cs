...
{
    public partial class OPDEmergencyPayOutMST : BasePage
    {
        ...
        private void bindddl()
        {
            ...
            Commonfunction.PopulateDdl(ddl_group, mstlookup.GetLookupsList(LookupName.OPpayoutGroup));
        ...
        protected void ddl_doctor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                GvOPDEmergency.Visible = true;
                GvOPDEmergency.DataSource = null;
                GvOPDEmergency.DataBind();
            }
        }
        ...
        private void bindgrid()
        {
            try
            {
                ...
                List<OPDEmergencyPayOutData> lstemp = GetOPDEmergencyPayOutDetails(0);
                if (lstemp.Count > 0)
                {
                    GvOPDEmergency.DataSource = lstemp;
                    GvOPDEmergency.DataBind();
                    GvOPDEmergency.Visible = true;
        ...
        private List<OPDEmergencyPayOutData> GetOPDEmergencyPayOutDetails(int p)
        {
            ...
            return objBO.SearchOPDEmergencyPayOutDetails(objData);
        }
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                OPDEmergencyPayOutBO objBO = new OPDEmergencyPayOutBO();
                OPDEmergencyPayOutData objData = new OPDEmergencyPayOutData();
                foreach (GridViewRow row in GvOPDEmergency.Rows)
                {
                    Label docid = (Label)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("lblDoctorID");
                    Label serviceid = (Label)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("lbl_serviceID");
                    TextBox rate = (TextBox)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("txtrate");
                    DropDownList sharetype = (DropDownList)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("ddl_sharetype");
                    TextBox hosshare = (TextBox)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("txt_hosshare");
                    TextBox conshare = (TextBox)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("txt_consultshare");
                    TextBox VaildFrom = (TextBox)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("txt_validfrom");
                    TextBox VaildTo = (TextBox)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("txt_validto");
                    CheckBox chk = (CheckBox)GvOPDEmergency.Rows[row.RowIndex].Cells[0].FindControl("chekbox");
                    ...
                    obj1.Validfrom = Convert.ToInt32(VaildFrom.Text == "" ? "0" : VaildFrom.Text);
                    obj1.Validto = Convert.ToInt32(VaildTo.Text == "" ? "0" : VaildTo.Text);
                    if (sharetype.SelectedIndex == 1)
                    {
                        ...
                        {
                            obj1.HospitalShare = Convert.ToDecimal(hosshare.Text == "" ? "0" : hosshare.Text);
                            obj1.ConsultantShare = Convert.ToDecimal(conshare.Text == "" ? "0" : conshare.Text);
                        }
                        else
                        {
                    ...
                    if (Convert.ToInt32(VaildFrom.Text == "" ? "0" : VaildFrom.Text) > Convert.ToInt32(VaildTo.Text == "" ? "0" : VaildTo.Text))
                    {
                        Messagealert_.ShowMessage(lblmessage, "Validdays", 0);
                        divmsg1.Visible = true;
                        divmsg1.Attributes["class"] = "FailAlert";
                        sharetype.Focus();
                        return;
                ...
                int result = objBO.UpdateOPDEmergency(objData);
                if (result == 1)
                {
                    bindgrid();
        ...
        protected void ExportoExcel()
        {
            ...
            {
                wb.Worksheets.Add(dt, "Consultant Payout");
                Response.Clear();
                Response.Buffer = true;
                Response.Charset = "";
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                Response.AddHeader("content-disposition", "attachment;filename=OPDEmergencyPayOutDetails.xlsx");
        ...
        {
            List<OPDEmergencyPayOutData> OPDEmergencyDetails = GetOPDEmergencyPayOutDetails(0);
